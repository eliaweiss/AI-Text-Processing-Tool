...
const variationsInput = document.getElementById(
  "variations-input"
) as HTMLInputElement;
...
function createVariationCard(index: number, text: string): HTMLDivElement {
  ...
  card.innerHTML = `
    <div class="variation-header">
      ...
      <button class="copy-variation-btn" data-text="${encodeURIComponent(
        text
      )}">
        📋 Copy
      </button>
  ...
  copyBtn.addEventListener("click", () =>
    copyVariationToClipboard(copyBtn, text)
  );
 ...
 */
async function copyVariationToClipboard(
    ...
    for (let i = 0; i < numVariations; i++) {
      ...
      if (result.success && result.processedText) {
        ...
        errorCard.innerHTML = `
          ...
          <div class="variation-content">Error: ${
...
function toggleAdvancedOptions(): void {
  const advancedGroups = operationSelect.querySelectorAll(".advanced-group");
  const isChecked = advancedCheckbox.checked;

  advancedGroups.forEach((group) => {
    (group as HTMLOptGroupElement).style.display = isChecked ? "" : "none";
  });
}
